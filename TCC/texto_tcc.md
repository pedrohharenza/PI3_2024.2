...
A primeira parte do sistema é responsável pelo controle da estação de recarga, que é realizado por meio da implementação de uma máquina de estados no microcontrolador STM32F0. Este microcontrolador foi escolhido por ser uma solução simples, de baixo custo e com ampla disponibilidade no mercado, além de atender plenamente às necessidades da aplicação. Para o controle da comutação de energia, foram selecionados dois relés, um para a fase e outro para o neutro. Os relés utilizados são do modelo HF161F da Hongfa, que possuem capacidade para suportar até 20A em 250VAC e corrente de acionamento de 75mA, atendendo assim a todos os requisitos necessários para a aplicação. A escolha de utilizar relés em uma estação de recarga monofásica é justificada principalmente pelo custo e tamanho reduzido, quando comparado com contactores. Além disso, eles oferecem menor consumo de energia e aquecimento, são compactos e exigem menos manutenção, o que os torna ideais para a aplicação. A comunicação entre os microcontroladores STM32F0 e STM32F373 foi projetada para utilizar UART (Universal Asynchronous Receiver-Transmitter), garantindo uma troca de dados eficiente e confiável.

A segunda parte do sistema é dedicada à aquisição de medidas de tensão elétrica e corrente elétrica, essenciais para o cálculo preciso do consumo de energia. Para essa finalidade, foi utilizado o microcontrolador STM32F373, que possui características avançadas voltadas para instrumentação, como um módulo de ponto flutuante e um conversor analógico-digital (ADC) do tipo Sigma-Delta. Esses recursos permitem medições de alta precisão, atendendo ao requisito de erro total inferior a 1%. O sensor de corrente elétrica utilizado foi um resistor shunt, que tem um ótimo desempenho quando utilizado junto a um ADC do tipo Sigma-Delta, enquanto a tensão elétrica da rede foi medida por meio de um divisor resistivo. Diferente da primeira parte, este circuito não é isolado da rede elétrica, o que exigiu cuidados adicionais no projeto para garantir a confiabilidade do sistema.
