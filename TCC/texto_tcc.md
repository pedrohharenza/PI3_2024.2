...
Para o realização da comutação de energia da estção de recarga, foram selecionados dois relés, um para a fase e outro para o neutro. Os relés utilizados são do modelo HF161F da Hongfa, que possuem capacidade para suportar até 20A em 250VAC e corrente de acionamento de 75mA, atendendo assim a todos os requisitos necessários para a aplicação. A escolha de utilizar relés em uma estação de recarga monofásica é justificada principalmente pelo custo e tamanho reduzido, quando comparado à utilização de contactores. A Figura X apresenta o circuito utilizado para o controle do acionamento dos relés via microcontrolador.
...
A segunda parte do sistema é dedicada à aquisição de medidas de tensão e corrente elétrica, essenciais para o cálculo do consumo de energia. Para essa finalidade, foi utilizado o microcontrolador STM32F373, que possui características voltadas para instrumentação, como um módulo de ponto flutuante, que garante desempenho de processamento, e um ADC do tipo sigma-delta. Esses recursos permitem medições que atendem ao requisito de erro total inferior a 1%. Para a aquisiçao de medidas o sensor utilizado para medir a corrente elétrica selecionado, foi um resistor shunt, que tem um ótimo desempenho quando utilizado junto a um ADC do tipo sigma-delta, enquanto a medida de tensão elétrica foi realizada por meio de um divisor resistivo. Diferente da primeira parte, este circuito não é isolado da rede elétrica, visto que a medida de tensão é em relação ao neutro, o que exigiu cuidados adicionais em relação a segurança do projeto. Para realizar a comunicação entre os microcontroladores STM32F0 e STM32F373, foi utilizada a interface UART (Universal Asynchronous Receiver-Transmitter), com isolamento garantido pelo circuito integrado ISO7421D da Texas Instruments.

Para atender ao funcionamento da estação de recarga, foram utilizados dois módulos de fonte chaveada isolados de 12V, conectados de forma a produzir um sinal de +12V e outro de -12V necessários para o protocolólo de comunicação da estação de rearga, seguido de estágios de redução para 5V e 3.3 V. Para a parte não isolada do circuito, foi empregado um módulo de fonte chaveada de 5V, seguido por um estágio de redução de tensão para 3.3 V. As Figuras X e X ilustram o esquemático de como foi realizada a implementação da alimentação da estação.
